...
const SIMPLIFY_RESPONSE = Symbol('SIMPLIFY_RESPONSE');
...
module.exports = class MagaClient {
   ...
   * @param {Boolean} [options.simplify] - whether simplify response data to developer, extract `result` property.
  ...
  * request(config) {
    ...
    try {
      ...
      try {
        ...
        // 对返回的响应数据，进行简化处理
        if (this.defaults.simplify) {
          // 覆盖掉 response.data
          response.data = protocolData;
          return this[SIMPLIFY_RESPONSE](response);
        }

        // 返回原来的数据, 加上 headers
        protocolData.headers = headers;
        return protocolData;
